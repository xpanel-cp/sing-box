...
import (
	...
	"github.com/sagernet/sing-box/experimental/sessionadmission/remotestore"
...
func New(options Options) (*Box, error) {
	...
	// In-core session admission (experimental.session_admission). Wired only when
	// the block is present and enabled; otherwise r.admission stays nil and the
	// router behaves byte-for-byte identically to upstream sing-box (R9.1/9.2).
	if experimentalOptions.SessionAdmission != nil && experimentalOptions.SessionAdmission.Enabled {
		saOptions := common.PtrValueOrDefault(experimentalOptions.SessionAdmission)
		saUsers := make([]sessionadmission.UserLimit, 0, len(saOptions.Users))
		for _, saUser := range saOptions.Users {
			saUsers = append(saUsers, sessionadmission.UserLimit{
				UUID:       saUser.UUID,
				MaxDevices: saUser.MaxDevices,
			})
		}
		admissionOptions := sessionadmission.Options{
			Enabled:         saOptions.Enabled,
			DefaultLimit:    saOptions.DefaultLimit,
			FailureMode:     saOptions.FailureMode,
			SlotGracePeriod: saOptions.SlotGracePeriod,
			Users:           saUsers,
		}
		// Select the SessionStore backend: a central RemoteStore when a manager
		// endpoint is configured (global, multi-node enforcement), otherwise the
		// per-node in-memory MemoryStore. The Gate and router call sites are
		// identical either way (interface parity).
		var admissionStore sessionadmission.SessionStore
		if saOptions.ManagerEndpoint != "" {
			remoteStore, rsErr := remotestore.New(remotestore.Config{
				Endpoint:    saOptions.ManagerEndpoint,
				NodeID:      saOptions.NodeID,
				NodeSecret:  saOptions.NodeSecret,
				MaxSessions: saOptions.MaxSessions,
			})
			if rsErr != nil {
				return nil, E.Cause(rsErr, "create session admission remote store")
			}
			admissionStore = remoteStore
			// Tie the RemoteStore heartbeat/connection lifecycle to the box: its
			// Close (stop heartbeat + close conn) runs on the normal box shutdown.
			internalServices = append(internalServices, adapter.NewLifecycleService(remoteStore, "session-admission-remote"))
			logFactory.NewLogger("session-admission").Info(
				"central session manager enabled (endpoint=", saOptions.ManagerEndpoint,
				", node_id=", saOptions.NodeID, ", max_sessions=", saOptions.MaxSessions, ")",
			)
		} else {
			admissionStore = sessionadmission.NewMemoryStore(admissionOptions.GracePeriod())
		}
		admissionLimits := sessionadmission.NewLimitSource(admissionOptions)
		admissionGate := sessionadmission.NewGate(sessionadmission.SourceIPFingerprinter{}, admissionStore, admissionLimits, admissionOptions.FailureModeValue())
		router.SetAdmissionGate(admissionGate)
		logFactory.NewLogger("session-admission").Info(
			"in-core session admission enabled (failure_mode=", admissionOptions.FailureModeValue(),
			", slot_grace_period=", admissionOptions.GracePeriod(), ")",
		)
	}
