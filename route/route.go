...
// chainClose composes release into an existing N.CloseHandlerFunc so the
// admission slot is freed when the connection closes (R5.1), while preserving
// the original close behavior. release runs first, then the original handler
// (if any) is invoked with the same error.
func chainClose(orig N.CloseHandlerFunc, release func()) N.CloseHandlerFunc {
...
// admitConnection runs the session admission gate before route selection. It is
// the single shared admission path for both the stream (RouteConnectionEx) and
// packet (RoutePacketConnectionEx) entry points, so production and tests
// exercise identical logic. Both net.Conn and N.PacketConn satisfy io.Closer.
//
//   - When no gate is configured (r.admission == nil) it is a no-op: the
//     original onClose is returned unchanged and ok=true, so behavior is
//     byte-for-byte identical to upstream sing-box (R9.1, R9.2).
//   - On a reject decision it logs exactly one rejection record carrying the
//     reason code (R2.4), closes the accepted inbound connection via the
//     standard handshake-failure idiom so the inbound performs one terminal
//     close having transferred zero payload bytes (R1.2, R2.2), and returns
//     ok=false so the caller returns BEFORE route selection or any outbound
//     dial (R1.1, R1.3).
//   - On an admit decision it chains the store release onto onClose so the slot
//     frees when the connection closes (R5.1), and returns ok=true.
func (r *Router) admitConnection(ctx context.Context, closer io.Closer, metadata adapter.InboundContext, onClose N.CloseHandlerFunc) (N.CloseHandlerFunc, bool) {
	if r.admission == nil {
		return onClose, true
	}
	// The Gate wraps ctx with its own 500ms admission deadline; the router
	// passes only its per-connection context and never sets a timeout.
	release, res := r.admission.Admit(ctx, metadata)
	if !res.Admit {
		r.logger.WarnContext(ctx, "session admission rejected: user=", metadata.User, " reason=", res.Reason)
		N.CloseOnHandshakeFailure(closer, onClose, E.New("session admission: ", string(res.Reason), " for ", metadata.User))
		return nil, false
	}
	return chainClose(onClose, release), true
}

func (r *Router) RouteConnectionEx(ctx context.Context, conn net.Conn, metadata adapter.InboundContext, onClose N.CloseHandlerFunc) {
	// Admission guard runs BEFORE routeConnection so a reject never reaches route
	// selection or the dial path and transfers zero bytes (R1.1, R1.3).
	onClose, ok := r.admitConnection(ctx, conn, metadata, onClose)
	if !ok {
		return
	}
...
func (r *Router) RoutePacketConnectionEx(ctx context.Context, conn N.PacketConn, metadata adapter.InboundContext, onClose N.CloseHandlerFunc) {
	// Identical guard to the stream path (R3): the same shared admitConnection
	// runs BEFORE routePacketConnection so UDP/QUIC rejects also transfer zero
	// bytes and never reach the dial path (R3.1, R3.2, R3.3).
	onClose, ok := r.admitConnection(ctx, conn, metadata, onClose)
	if !ok {
		return
	}
