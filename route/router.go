...
// admissionGate is the router-local seam onto the session admission core. It is
// intentionally defined in the route package (rather than importing the Gate
// concretely at call sites) so the router stays decoupled and easily testable:
// production wiring injects a *sessionadmission.Gate, while tests can inject a
// lightweight stub. It mirrors sessionadmission.Gate.Admit.
type admissionGate interface {
	Admit(ctx context.Context, md adapter.InboundContext) (release func(), result sessionadmission.AdmissionResult)
}
...
// SetAdmissionGate injects the session admission gate. It is left nil by
// default so that, when no session_admission config block is present, the
// router behaves byte-for-byte identically to upstream sing-box (R9.1).
// Startup wiring (a later task) calls this only when the gate is configured.
func (r *Router) SetAdmissionGate(g admissionGate) {
	r.admission = g
}
