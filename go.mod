...
// X-NET: sing-box's common/badtls uses //go:linkname into crypto/tls internals
// that changed in Go 1.26 (link error: invalid reference to
// crypto/tls.(*Conn).handlePostHandshakeMessage). Go 1.25.x is the supported
// toolchain (the fork Dockerfile pins golang:1.25). This line documents that
// floor; note it does NOT cap the toolchain — a host with a NEWER Go (e.g.
// 1.26) under GOTOOLCHAIN=auto/local still uses the newer one. To FORCE 1.25 on
// such a host, build via scripts/build-xnet-singbox.{sh,ps1} (which sets
// GOTOOLCHAIN=go1.25.x) or build on a Go 1.25 host / the fork Dockerfile.
toolchain go1.25.0
